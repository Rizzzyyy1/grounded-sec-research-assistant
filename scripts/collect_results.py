...
def citation_hygiene_section() -> list[str]:
    ...
    return [
        ...
        "row's test column reflects four further fixes** (`agent/tools.py::_register_fact` giving "
        "XBRL facts a real citation; a `computed_metric` tool-selection fix; "
        "`generation/citations.py::attribute_claims` giving a genuinely-paraphrased-but-unbracketed "
        "claim a real citation instead of leaving it uncited - ERROR_ANALYSIS.md 3d/3e/3f; then a "
        "manual audit of that mechanism against the full underlying evidence, which found and fixed "
        "one confirmed false attribution and a separate warning-suppression bug, tightening what "
        "counts as support - ERROR_ANALYSIS.md 3g): test accuracy rose 0.735 -> 0.853 and citation "
        "hygiene 0.0% -> 32.5% with zero new regressions at any stage (paired per-question diff "
...
def ollama_agent_section() -> list[str]:
    ...
    if stats_lines:
        lines += [
            "On the historical 38-question natural-phrasing probe (paired runs used the same "
            "question file at the time; the draft questions and labels have since been revised, "
            "so these in-sample scores do not describe the revised file):",
            "",
            *stats_lines,
            "",
        ]
...
def natural_section() -> list[str]:
    ...
    return [
        "",
        "### Templated vs natural phrasing (historical `gold_v2_draft`, 38 questions)",
        "",
        ...
        *rows,
        ...
        "in that historical file were unverified draft judgement. The draft has since been edited "
        "to clarify ambiguous questions and remove three broad retrieval labels; the displayed "
        "scores and n=12 retrieval count are historical and must not be reused as results for the "
        "revised draft. See docs/BENCHMARK_REVIEW_DECISIONS.md.",
    ]
...
def main() -> None:
    if not any((REPORTS / "runs").glob("*/summary.md")) and (REPORTS / "RESULTS.md").exists():
        raise SystemExit(
            "no raw evaluation runs are available in this checkout; refusing to replace "
            "published results with placeholders. Restore the recorded runs or run new evaluations."
        )
    body = build()
