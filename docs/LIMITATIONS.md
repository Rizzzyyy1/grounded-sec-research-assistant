...
| Local-model quality | `llama3.2:3b` (the free `--llm ollama` default) still sometimes cites a source_id no tool ever produced, forgets to bracket a claim entirely (even a genuine paraphrase of a retrieved passage - now recovered automatically, see below), answers a qualitative question from training-data familiarity instead of the retrieved passages (a real gap: no shared vocabulary with anything retrieved, so nothing can be attributed to it), or narrates a fake tool call as plain text instead of issuing one - observed on different questions on different runs, i.e. intermittent, not tied to one question. On a comparative question where only one side is citable, it can also hedge into an abstention while still (correctly, without fabrication) stating both real numbers - a safer failure than before, but still scored incorrect by a strict grader (ERROR_ANALYSIS 3d, `nat-cmp-016`) | Documented, not patched around (ERROR_ANALYSIS 3c/3d); an invalid citation is never shown to the user as if valid (`generation/citations.py::repair_citations`), a forgotten-but-genuinely-supported bracket is added automatically (`attribute_claims`, ERROR_ANALYSIS 3f), and the rest are caught as warnings; a bigger local model (`FINSIGHT_OLLAMA__MODEL`) or `--llm claude` are the two ways to reduce the remainder, neither measured here |
...
| Reproducibility | A local model's default sampling gave a different tool call for an identical question on consecutive runs; separately, the same question through the same deterministic-sampling settings has also returned different retrieved passages and different `S`-numbering across separate live invocations (ERROR_ANALYSIS 3g) | `generation/ollama.py` sets `temperature=0, seed=0`; confirmed deterministic for tool selection, though still not bit-exact for retrieval/embedding across every backend/batch size. A specific `S`-number from one run is not guaranteed to mean the same evidence in another |
| Citation attribution precision | Lexical overlap alone let one confirmed false attribution through (a compound sentence where only one clause was genuinely sourced, attached because the *sentence as a whole* shared enough incidental vocabulary with an off-topic passage); a shared non-financial number ("ISO 27001") could also silently excuse an entire unbracketed, partly-unsupported claim from the uncited-claim warning | Audited manually against the full underlying evidence, not the trimmed display quote (ERROR_ANALYSIS 3g). Fixed with a higher overlap threshold, a contiguous-phrase requirement, an explicit direction-reversal veto, a fiscal-year-mismatch veto, and a stricter grounding-figure check - all tested against both a supporting and a contradicting case. The `Citation.quote` shown in the UI is still trimmed to 280 characters from the start of the chunk, so a reader can see a citation that looks unsupported by its own displayed quote even when the full underlying passage genuinely supports the claim; not fixed this pass |
