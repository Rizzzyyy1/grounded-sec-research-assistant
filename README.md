...
A chatbot answering *"How did Apple's gross margin change over three years and why?"* can return
unsupported figures, incorrect calculations, or missing citations. Financial research needs traceable evidence.
...
Result tables below were collected by `scripts/collect_results.py` from evaluation artifacts.
Raw per-run files in `reports/runs/` are not included in this public repository, so a fresh clone
...
On the 38-question natural-phrasing probe (used to develop fixes; this comparison is in-sample, not an independent holdout):
...
| System | `gold_v1` templated | natural, **before** the guardrail fix (historical) | natural, after the fix (**in-sample**) |
|---|---|---|---|
...
8. **The extractive baseline over-answers.** It quotes any passage sharing words with an unanswerable
   ...
   abstention behaviour is meant to fix — local-agent abstention results are reported above; Claude-specific behavior remains unmeasured.
...
```bash
git clone https://github.com/Rizzzyyy1/grounded-sec-research-assistant.git
cd grounded-sec-research-assistant
...
* **Decisions recorded** — [Architecture Decision Records](docs/adr/) with alternatives and consequences.
...
[Project overview](docs/PORTFOLIO.md) · [Load test](reports/load_test.md)

## Development process

Developed with substantial AI assistance, including Claude Code; commit co-author attribution is
retained. See the [project overview](docs/PORTFOLIO.md) for implementation evidence and limits,
and the [dataset integrity record](docs/DATASET_INTEGRITY.md) for evaluation exposure.
